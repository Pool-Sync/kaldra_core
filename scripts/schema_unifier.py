...
def main():
    ...
    if archetypes_data:
        schema = infer_schema_from_list(
            archetypes_data,
            "KALDRA Archetypes",
            "Schema for the 12 primary archetypes.",
        )
    ...
    if modifiers_data:
        ...
        if isinstance(modifiers_data, dict) and "modifiers" in modifiers_data:
            schema = infer_schema_from_list(
                ...
                "KALDRA Modifiers",
                "Schema for archetype modifiers.",
            )
    ...
    if polarities_data:
        if isinstance(polarities_data, dict) and "polarities" in polarities_data:
            schema = infer_schema_from_list(
                polarities_data["polarities"],
                "KALDRA Polarities",
                "Schema for polarities.",
            )
    ...
    tau_schema = {
        ...
        "type": "object",
        "properties": {"tau_config": {"type": "object"}, "policies": {"type": "array"}},
    }
