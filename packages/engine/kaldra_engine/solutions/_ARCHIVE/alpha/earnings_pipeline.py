...
def run_earnings_pipeline(
    ...
    if use_fallback_embeddings:
        router_config = EmbeddingRouterConfig(provider="fallback", dim=target_dim, use_fallback=True)
    else:
        router_config = EmbeddingRouterConfig(provider="sentence-transformers", dim=target_dim, use_fallback=True)
    ...
    metadata = {
        ...
        "epistemic_status": getattr(signal.epistemic, "status", "UNKNOWN")
        if hasattr(signal, "epistemic")
        else "UNKNOWN",
    }
