...
class ProductNarrativeInput:
    ...
    channel: str | None = None  # "review", "social", "support", etc.
...
def map_text_to_product_kindra(
    ...
    router_config = EmbeddingRouterConfig(provider="fallback", dim=engine.d_ctx, use_fallback=True)
    ...
    if archetype_top_indices:
        ...
        kindra_layer1 = {f"vector_{i}": float(probs[i]) for i in archetype_top_indices[:3]}
        dominant_vectors = [f"vector_{archetype_top_indices[0]}"]

    logger.info(f"Product Kindra mapping complete: product_id={product_id}, top_archetypes={archetype_top_indices[:3]}")
