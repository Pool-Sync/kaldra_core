...
class Explanation:
    ...
    def __init__(self, summary: str, details: dict[str, Any], raw_facts: dict[str, Any]):
        ...
        self.confidence: Any | None = None  # ExplanationConfidence
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.trace:
            result["trace"] = [asdict(step) if hasattr(step, "__dataclass_fields__") else step for step in self.trace]

        return result

    def to_markdown(self, renderer: Any | None = None, variant: str = "default") -> str:
        """
        ...
        """
        try:
            from kaldra_engine.explainability.explanation_output import (
                ExplanationMarkdownRenderer,
            )
...
class ExplanationGenerator:
    ...
    def __init__(
        ...
        confidence_engine: Any | None = _DEFAULT_CONFIDENCE,
    ):
        """
    ...
    def _load_default_template(self) -> dict[str, str]:
        ...
        template_path = os.path.join(os.path.dirname(__file__), "templates", "base_template.md")
        ...
        if self.confidence_engine is not None:
            try:
                ...
                explanation.details["confidence"] = {
                    ...
                    "components": [{"name": c.name, "score": c.score, "reason": c.reason} for c in conf.components],
                }
            except Exception as e:
    ...
    def _generate_with_llm(self, facts: dict[str, Any]) -> Explanation:
        ...
        return Explanation(summary=summary, details=details, raw_facts=facts)
