...
def explanation_to_proto(explanation: Any) -> ExplanationProto:
    ...
    details_map = {k: json.dumps(v) if not isinstance(v, str) else v for k, v in explanation.details.items()}
    raw_facts_map = {k: json.dumps(v) if not isinstance(v, str) else v for k, v in explanation.raw_facts.items()}

    proto_msg = ExplanationProto(summary=explanation.summary, details=details_map, raw_facts=raw_facts_map)
...
def explanation_from_proto(proto_msg: ExplanationProto) -> dict[str, Any]:
    ...
    result = {"summary": proto_msg.summary, "details": details, "raw_facts": raw_facts}
