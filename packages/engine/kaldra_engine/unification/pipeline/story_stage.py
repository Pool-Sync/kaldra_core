...
from kaldra_engine.common.unified_signal import (
    StoryEvent,
)

# Use unified_signal StoryEvent (has stream_id)
from kaldra_engine.story.arc_detector import ArcDetector
from kaldra_engine.story.coherence_scorer import CoherenceScorer
from kaldra_engine.story.story_buffer import StoryBuffer, StoryBufferConfig
from kaldra_engine.story.timeline_builder import TimelineBuilder

from ..states.unified_state import StoryContext, UnifiedContext
...
class StoryStage:
    ...
    def __init__(self, config: StoryStageConfig | None = None):
        """
        ...
        logger.info(
            f"StoryStage initialized: max_events={self.config.max_events}, window_size={self.config.window_size}"
        )
    ...
    def run(self, context: UnifiedContext) -> UnifiedContext:
        ...
        try:
            ...
            logger.info(
                f"StoryStage: processed {len(events_window)} events, arc={arc.dominant_stage if arc else 'None'}"
            )
    ...
    def _build_event_from_context(self, context: UnifiedContext) -> StoryEvent:
        ...
        if context.archetype_ctx:
            if context.archetype_ctx.delta12:
                ...
                delta12 = (
                    context.archetype_ctx.delta12.to_dict() if hasattr(context.archetype_ctx.delta12, "to_dict") else {}
                )
        ...
        if context.drift_ctx:
            if context.drift_ctx.drift_state:
                drift_state = (
                    context.drift_ctx.drift_state.to_dict() if hasattr(context.drift_ctx.drift_state, "to_dict") else {}
                )
            if context.drift_ctx.tw_state:
                tw_state = (
                    context.drift_ctx.tw_state.to_dict() if hasattr(context.drift_ctx.tw_state, "to_dict") else {}
                )
