...
class CoreStage:
    ...
    def execute(self, context: UnifiedContext) -> UnifiedContext:
        ...
        try:
            ...
            delta144_result = self.delta144.infer_from_vector(embedding, tau_modifiers=tau_modifiers)
            ...
            drift_ctx = DriftContext(regime=delta144_result.state.archetype_id, drift_metric=0.0)
            ...
            logger.info(
                f"Core stage complete: archetype={delta144_result.archetype.label}, state={delta144_result.state.label}"
            )
