...
from kaldra_engine.story.multi_stream_buffer import MultiStreamBuffer
from kaldra_engine.story.stream_comparator import StreamComparator

from ..states.unified_state import MultiStreamContext, UnifiedContext
...
class MultiStreamStage:
    ...
    def __init__(self, config: MultiStreamStageConfig | None = None):
        """
        ...
        logger.info(
            f"MultiStreamStage initialized: window_size={self.config.window_size}, enabled={self.config.enabled}"
        )
    ...
    def run(self, context: UnifiedContext) -> UnifiedContext:
        ...
        try:
            ...
            logger.info(
                f"MultiStreamStage: analyzed {len(active_streams)} streams, max_div={max_div:.3f}, convergent={convergent}"
            )
