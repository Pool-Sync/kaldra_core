...
def test_basic_parallel_execution():
    ...
    tasks = {"task_a": task_a, "task_b": task_b, "task_c": task_c}
    ...
    print("   Expected: ~50ms (parallel) vs ~150ms (sequential)")
...
def test_timeout_handling():
    ...
    tasks = {"fast": fast_task, "slow": slow_task}
    ...
    if results["fast"].status == TaskStatus.COMPLETED and results["slow"].status == TaskStatus.TIMEOUT:
...
def test_failure_isolation():
    ...
    tasks = {"good1": good_task_1, "bad": bad_task, "good2": good_task_2}
...
def test_context_passing():
    ...
    tasks = {"task1": task_with_context, "task2": task_with_context}
