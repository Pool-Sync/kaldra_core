...
from kaldra_engine.config import KINDRAS_48_FILE
...
class KaldraKindraCulturalMod(nn.Module):
    ...
    def __init__(self, d_ctx: int = 256):
        ...
        self.W = nn.ModuleDict({p: nn.Linear(d_ctx, 48) for p in ["3", "6", "9"]})

        # Matriz de mapeamento 48 (Kindras) → 144 (Estados Δ144)
        # Inicializada via schema (semântica) ou ruído se falhar
        self.M = nn.ParameterDict({p: nn.Parameter(torch.randn(48, 144) * 0.01) for p in ["3", "6", "9"]})
    ...
    def _init_semantic_matrix(self):
        ...
        """
        try:
            with open(KINDRAS_48_FILE, encoding="utf-8") as f:
