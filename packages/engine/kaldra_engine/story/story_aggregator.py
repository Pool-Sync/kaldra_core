...
class MotionVector:
    ...
    delta12_dominant_change: tuple[str, str] | None = None  # (from_archetype, to_archetype)

    # Δ144 motion
    delta144_transition: tuple[str, str] | None = None  # (from_state, to_state)
...
class StoryAggregation:
    ...
    arc_progression: ArcProgression | None
    drift_trajectory: DriftTrajectory | None
        ...
        for engine in ["nietzsche", "aurelius", "campbell"]:
            if engine in prev.meta_scores and engine in curr.meta_scores:
                ...
                if isinstance(prev_scores, dict) and isinstance(curr_scores, dict):
                    deltas = [
                        curr_scores.get(k, 0) - prev_scores.get(k, 0)
...
def detect_inflection_points(history: list[StoryEvent]) -> list[InflectionPoint]:
    ...
    for i in range(1, len(history)):
        ...
        if prev.meta_scores and curr.meta_scores and "campbell" in prev.meta_scores and "campbell" in curr.meta_scores:
...
def detect_arc_progression(buffer: StoryBuffer) -> ArcProgression | None:
    """
...
def _compute_drift_trajectory(timeline: list[StoryEvent]) -> DriftTrajectory | None:
