...
from kaldra_engine.meta.types import MetaInput  # Shared MetaInput
...
class AureliusEngine:
    ...
    def _calculate_virtue_scores(self, profile: AureliusProfile, kindra_sig: dict[str, float]) -> dict[str, float]:
        """
        ...
        wisdom = profile.perception_clarity * 0.4 + profile.assent_to_reality * 0.3 + profile.control_dichotomy * 0.3

        # Courage: Action in face of difficulty
        courage = profile.right_action * 0.4 + profile.discipline_of_will * 0.3 + profile.premeditatio_malorum * 0.3

        # Justice: Right action toward others
        justice = profile.right_action * 0.4 + profile.character_integrity * 0.3
        ...
        temperance = profile.desire_restraint * 0.4 + profile.emotional_regulation * 0.3 + profile.serenity * 0.3
    ...
    def _calculate_dichotomy_of_control(
        self, profile: AureliusProfile, kindra_sig: dict[str, float]
    ) -> dict[str, float]:
        """
        ...
        return {"controllable": controllable, "not_controllable": uncontrollable}

    def _calculate_memento_mori(self, profile: AureliusProfile, kindra_sig: dict[str, float]) -> float:
        """
    ...
    def _calculate_amor_fati(self, profile: AureliusProfile, kindra_sig: dict[str, float]) -> float:
        """
        ...
        - serenity
        """
        amor_fati = profile.fate_acceptance * 0.5 + profile.assent_to_reality * 0.3 + profile.serenity * 0.2
    ...
    def _adjust_for_archetypes(self, profile: AureliusProfile, archetype_scores: dict[str, float]) -> AureliusProfile:
        """
...
def analyze_meta(
    text: str,
    *,
    delta12: Any | None = None,
    delta144_state: str | None = None,
    tw_state: Any | None = None,
