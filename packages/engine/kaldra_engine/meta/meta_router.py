...
from .campbell import CampbellEngine
...
class MetaRouter:
    ...
    def __init__(self, engines: list[MetaEngineBase] | None = None):
        """
    ...
    def evaluate(self, signal: dict[str, Any]) -> dict[str, MetaSignal]:
        ...
        try:
            ...
            results["nietzsche"] = MetaSignal(name="nietzsche", score=0.5, label="error", details={"error": str(e)})
        ...
        try:
            ...
            results["aurelius"] = MetaSignal(
                ...
                label="regulated"
                if aurelius_result.severity > 0.6
                else "on_the_edge"
                if aurelius_result.severity > 0.4
                else "reactive",
                details=aurelius_result.scores,
            )
        except Exception as e:
            results["aurelius"] = MetaSignal(name="aurelius", score=0.5, label="error", details={"error": str(e)})
    ...
    def decide_route(self, meta_signals: dict[str, MetaSignal]) -> RoutingDecision:
        ...
        if campbell and campbell.label in [
            "ordeal",
            "approach_to_cave",
