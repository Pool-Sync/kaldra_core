...
class CampbellEngine:
    ...
    def _infer_journey_sequence_from_story(self, story_ctx: Optional["StoryContext"]) -> list[str]:
        """
    ...
    def _measure_transformation_arc(
        ...
        delta144_timeline: list[dict[str, Any]] | None,
    ) -> float:
        """
    ...
    def _compute_temporal_coherence(self, story_ctx: Optional["StoryContext"]) -> float:
        """
    ...
    def _compute_drift_coupling(self, drift_ctx: Optional["DriftContext"], journey_sequence: list[str]) -> float:
        """
    ...
    def analyze(self, meta_input: MetaInput) -> CampbellSignal:
        ...
        archetypal_roles, active_archetypes = self._map_delta144_to_roles(
            meta_input.delta144_state, meta_input.archetype_scores
        )
        ...
        journey_stage, stage_conf = self._detect_journey_stage(text_lower, archetypal_roles, kindra_sig)

        # 4. Estimate Transformation Potential (TW369)
        trans_potential = self._estimate_transformation_potential(journey_stage, kindra_sig, meta_input.tw_state)

        # 5. Compute Mythic Resonance
        mythic_resonance = self._compute_mythic_resonance(journey_stage, stage_conf, archetypal_roles, kindra_sig)
        ...
        try:
            ...
            if story_ctx or delta144_timeline:
                signal.arc_completeness = self._measure_transformation_arc(
                    story_ctx=story_ctx, delta144_timeline=delta144_timeline
                )

            # (d) Drift coupling
            if drift_ctx and signal.journey_sequence:
                signal.drift_coupling = self._compute_drift_coupling(
                    drift_ctx=drift_ctx, journey_sequence=signal.journey_sequence
                )
    ...
    def _map_delta144_to_roles(
        self, delta144_state: str | None, archetype_scores: dict[str, float]
        ...
        if archetype_scores:
            ...
            for arch_id, _score in sorted_archs[:3]:  # Take top 3
    ...
    def _compute_kindra_mythic_signature(self, kindra: KindraContext | None) -> dict[str, float]:
        """
    ...
    def _detect_journey_stage(
        self, text: str, roles: dict[str, str], kindra_sig: dict[str, float]
    ) -> tuple[str, float]:
        """
