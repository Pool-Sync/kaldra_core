...
def test_narrative_risk_range():
    ...
    response = client.post("/engine/kaldra/signal", json={"text": "Technology evolves rapidly"})
...
def test_narrative_risk_high_bias():
    ...
    # Low bias text
    response_low = client.post("/engine/kaldra/signal", json={"text": "A simple observation about nature."})
...
def test_narrative_risk_formula():
    ...
    response = client.post("/engine/kaldra/signal", json={"text": "Change brings uncertainty"})
    ...
    expected_risk = 0.4 * bias_score + 0.3 * tw_factor + 0.3 * (1.0 - confidence)
...
def test_narrative_risk_tw_trigger():
    ...
    response = client.post("/engine/kaldra/signal", json={"text": "Volatility and instability"})
...
def test_narrative_risk_confidence_impact():
    ...
    response = client.post("/engine/kaldra/signal", json={"text": "Ambiguous and uncertain situation"})
...
def test_narrative_risk_consistency():
    ...
    risks = []
    for _ in range(3):
        response = client.post("/engine/kaldra/signal", json={"text": text})
        data = response.json()
