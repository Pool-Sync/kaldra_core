...
def test_bias_neutral_text():
    ...
    response = client.post("/engine/kaldra/signal", json={"text": "The weather is nice today."})
...
def test_bias_charged_text():
    ...
    response = client.post("/engine/kaldra/signal", json={"text": "THIS IS THE WORST DISASTER EVER!!!"})
...
def test_bias_label_coherence():
    ...
    # Low bias
    response_low = client.post("/engine/kaldra/signal", json={"text": "A simple statement."})
    data_low = response_low.json()

    # High bias
    response_high = client.post("/engine/kaldra/signal", json={"text": "ABSOLUTELY TERRIBLE CATASTROPHE!!!"})
...
def test_bias_fields_required():
    ...
    response = client.post("/engine/kaldra/signal", json={"text": "Any text here"})
