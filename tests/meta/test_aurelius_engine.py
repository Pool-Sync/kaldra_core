...
from meta.aurelius import AureliusEngine, AureliusSignal, MetaEngineResult, analyze_meta
...
def test_uses_kindra_signature_when_present():
    ...
    kindra_high_responsibility = KindraContext(
        layer1={"duty": 0.9, "responsibility": 0.8, "collective_action": 0.85},
        layer2={"calm": 0.7, "measured": 0.6},
        layer3={},
    )
    meta_input_with_kindra = MetaInput(text="I focus on my duty and responsibility.", kindra=kindra_high_responsibility)
...
def test_kindra_emotional_volatility_reduces_temperance():
    ...
    kindra_volatile = KindraContext(
        layer1={},
        layer2={"emotional_intensity": 0.9, "reactivity": 0.8, "passion": 0.85},
        layer3={},
    )

    meta_input = MetaInput(text="I remain calm and composed.", kindra=kindra_volatile)
    result = engine.analyze(meta_input)
...
def test_kindra_existential_depth_boosts_memento_mori():
    ...
    kindra_existential = KindraContext(
        layer1={},
        layer2={},
        layer3={"mortality": 0.9, "meaning": 0.8, "existential": 0.85},
    )

    meta_input = MetaInput(text="We must prepare for difficulties.", kindra=kindra_existential)
    result = engine.analyze(meta_input)
...
def test_tw369_critical_regime_boosts_memento_mori():
    ...
    meta_input = MetaInput(text="We must prepare for the worst.", tw_state=tw_critical)
    result = engine.analyze(meta_input)
...
def test_tw369_transition_regime_boosts_amor_fati():
    ...
    meta_input = MetaInput(text="I accept what is and flow with change.", tw_state=tw_transition)
    result = engine.analyze(meta_input)
...
def test_tw369_stable_regime_boosts_virtues():
    ...
    meta_input = MetaInput(text="I act with wisdom, courage, justice, and temperance.", tw_state=tw_stable)
    result = engine.analyze(meta_input)
...
def test_virtue_scores_calculation():
    ...
    meta_input_courage = MetaInput(text="I do my duty with discipline and prepare for difficulties ahead.")
    ...
    meta_input_temperance = MetaInput(text="I am calm, moderate, and serene in all things.")
...
def test_dichotomy_of_control_detection():
    ...
    meta_input_controllable = MetaInput(text="I focus on what is in my control and within my power to change.")
    result_controllable = engine.analyze(meta_input_controllable)

    # Focus on uncontrollable
    meta_input_uncontrollable = MetaInput(text="Everything is beyond my control and outside my power.")
...
def test_dichotomy_with_kindra_control_focus():
    ...
    kindra_control = KindraContext(
        layer1={"control": 0.9, "agency": 0.8, "self_determination": 0.85},
        layer2={},
        layer3={},
    )

    meta_input = MetaInput(text="I focus on my actions.", kindra=kindra_control)
    result = engine.analyze(meta_input)
...
def test_memento_mori_calculation():
    ...
    meta_input = MetaInput(text="We must prepare for the worst case and anticipate difficulties.")
    result = engine.analyze(meta_input)
...
def test_amor_fati_calculation():
    ...
    meta_input = MetaInput(text="I accept my fate, embrace what is, and remain serene and peaceful.")
    result = engine.analyze(meta_input)
...
def test_stoic_posture_classification():
    ...
    assert result_exemplary.label in [
        "exemplary_stoic",
        "stoic",
        "mixed",
    ]  # Heuristic may classify as mixed

    # Non-Stoic
    meta_input_non_stoic = MetaInput(
        text="I'm outraged and furious! This is a catastrophe! I want more and more, never enough!"
    )
...
def test_anti_patterns_reduce_scores():
    ...
    meta_input_reactive = MetaInput(text="I'm outraged and furious! This is a catastrophe and disaster!")
    ...
    # Excess
    meta_input_excess = MetaInput(text="I want more and more, never enough, insatiable and greedy!")
