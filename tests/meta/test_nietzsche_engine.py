...
def test_uses_kindra_signature_when_present():
    ...
    kindra_high_power = KindraContext(
        layer1={"power_dynamics": 0.9, "dominance": 0.8, "hierarchy": 0.7},
        layer2={"conflict": 0.6},
        layer3={"archetypal_hero": 0.7},
    )
    meta_input_with_kindra = MetaInput(text="We will dominate the market.", kindra=kindra_high_power)
    ...
    kindra_high_ressentiment = KindraContext(
        layer1={"victimhood": 0.9, "grievance": 0.8, "resentment": 0.7},
        layer2={"victim_narrative": 0.8, "injustice_framing": 0.7},
        layer3={},
    )
    meta_input_ressentiment = MetaInput(text="It's so unfair.", kindra=kindra_high_ressentiment)
...
def test_kindra_mythic_intensity_boosts_transcendence():
    ...
    meta_input = MetaInput(text="We embrace our destiny with strength and wisdom.", kindra=kindra_mythic)
    result = engine.analyze(meta_input)
...
def test_tw369_critical_regime_boosts_nihilism():
    ...
    meta_input = MetaInput(text="We must tear down the old structures.", tw_state=tw_critical)
    result = engine.analyze(meta_input)
...
def test_tw369_transition_regime_boosts_transvaluation():
    ...
    meta_input = MetaInput(text="We are redefining our values.", tw_state=tw_transition)
    result = engine.analyze(meta_input)
...
def test_morality_type_classification_mixed():
    ...
    # Mixed signals
    meta_input = MetaInput(text="We have power but also feel resentment. We lead but also blame others.")
    result = engine.analyze(meta_input)
...
def test_morality_type_with_kindra_power_climate():
    ...
    kindra_power = KindraContext(layer1={"power_dynamics": 0.9, "dominance": 0.8}, layer2={}, layer3={})

    meta_input = MetaInput(text="We will succeed.", kindra=kindra_power)
    result = engine.analyze(meta_input)
...
def test_dionysian_vs_apollonian():
    ...
    meta_input_order = MetaInput(
        text="We need structure, order, and systematic organization. Harmony and balance are essential."
    )
    result_order = engine.analyze(meta_input_order)

    # Dionysian text
    meta_input_chaos = MetaInput(text="Wild chaos and primal passion! Embrace the frenzy and ecstasy of raw instinct!")
...
def test_active_vs_passive_nihilism():
    ...
    meta_input_active = MetaInput(
        text="Tear down the old structures! Destroy and rebuild. Creative destruction is necessary."
    )
...
def test_transcendence_calculation():
    ...
    meta_input = MetaInput(
        text="We embrace life with power and joy, free from resentment, creating our own values beyond tradition."
    )
    result = engine.analyze(meta_input)
...
def test_notes_detect_contradictions():
    ...
    meta_input = MetaInput(
        text="We have immense power and strength to dominate, but it's unfair and we're victims who deserve better."
    )
    result = engine.analyze(meta_input)
