...
def master_engine():
    ...
    with (
        patch("src.core.kaldra_master_engine.Delta144Engine"),
        patch("src.core.kaldra_master_engine.KaldraKindraCulturalMod"),
        patch("src.core.kaldra_master_engine.TWPainleveOracle"),
        patch("src.core.kaldra_master_engine.TW369Integrator"),
    ):
        ...
        engine.kindra_mod.return_value = (
            MagicMock(detach=lambda: MagicMock(cpu=lambda: MagicMock(numpy=lambda: np.ones(144) / 144))),
        )
