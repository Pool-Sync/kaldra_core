...
def test_detect_inflection_points_archetype_shift():
    ...
    buffer.add_event("Event 1", delta12={"A01_INNOCENT": 0.9, "A02_ORPHAN": 0.1})
    buffer.add_event(
        "Event 2",
        delta12={"A01_INNOCENT": 0.2, "A03_WARRIOR": 0.8},  # Major shift
    )
    buffer.add_event("Event 3", delta12={"A03_WARRIOR": 0.85, "A01_INNOCENT": 0.15})
...
def test_detect_arc_progression():
    ...
    buffer.add_event(
        "Event 2",
        meta_scores={"campbell": {"stage": "call_to_adventure", "confidence": 0.7}},
    )
    buffer.add_event(
        "Event 3",
        meta_scores={"campbell": {"stage": "tests_allies_enemies", "confidence": 0.75}},
    )
...
def test_drift_trajectory_computation():
    ...
    for i in range(5):
        buffer.add_event(f"Event {i}", drift_state={"drift_metric": 0.2 + i * 0.1})
...
def test_meta_score_deltas():
    ...
    event1 = buffer.add_event("Event 1", meta_scores={"nietzsche": {"will_to_power": 0.5, "amor_fati": 0.4}})

    event2 = buffer.add_event("Event 2", meta_scores={"nietzsche": {"will_to_power": 0.8, "amor_fati": 0.6}})
