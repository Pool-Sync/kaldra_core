...
def mock_context():
    ...
    # Mock Kindra Context
    ctx.kindra_ctx = KindraContext(layer1={"order": 0.5}, layer2={"intensity": 0.8}, layer3={"myth": 0.9})

    # Mock Drift Context (which contains TWState)
    tw_state = TWState(metadata={"drift_metric": 0.5, "regime": "STABLE"})
    ctx.drift_ctx = DriftContext(tw_state=tw_state, drift_state=MagicMock(), regime="STABLE", drift_metric=0.5)
