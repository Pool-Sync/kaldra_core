...
def test_profile_creation_and_retrieval(tmp_path):
    ...
    mgr.create_profile("user_123", {"preferred_preset": "geo", "risk_tolerance": 0.8})
...
def test_profile_update(tmp_path):
    ...
    mgr.update_profile("user_123", {"risk_tolerance": 0.9, "output_format": "markdown"})
...
def test_preset_router_merges_profile_risk_and_output(tmp_path):
    ...
    profile_mgr.create_profile("user_1", {"risk_tolerance": 0.95, "output_format": "markdown"})
...
def test_preset_router_custom_preferences_merged(tmp_path):
    ...
    profile_mgr.update_profile("user_custom", {"custom_field": "custom_value", "favorite_domains": ["finance", "geo"]})
...
def test_full_flow_preset_to_profile_to_resolution(tmp_path):
    ...
    profile_mgr = ProfileManager(storage_dir=str(storage))
    profile_mgr.create_profile("analyst_001", {"preferred_preset": "alpha", "risk_tolerance": 0.75, "depth": "deep"})

    # 3. Resolve with router
    router = PresetRouter(preset_manager=preset_mgr, profile_manager=profile_mgr)
...
def test_preset_switch_maintains_profile(tmp_path):
    ...
    storage = tmp_path / "profiles"
    profile_mgr = ProfileManager(storage_dir=str(storage))
    profile_mgr.create_profile("switcher", {"risk_tolerance": 0.6, "output_format": "detailed"})
...
def test_emphasis_list_converted_to_weighted_dict():
    ...
    for _key, weight in resolved.emphasis.items():
        assert weight == 1.0
