...
def test_memory_keeps_window_size():
    ...
    # Add 10 states
    for i in range(10):
        state = DriftState(timestamp=float(i), drift_metric=float(i) * 0.1, regime=f"STATE_{i}")
        memory.append(state)
...
def test_save_and_load_file():
    ...
    # Add some states
    for i in range(3):
        state = DriftState(timestamp=float(i), drift_metric=float(i) * 0.2, regime=f"A0{i + 1}_TEST")
        memory.append(state)
