...
from kaldra_engine.tw369.drift_history import DriftHistory
...
class TestTracyWidomSeverity:
    ...
    def test_severity_monotonic_in_lambda(self):
        ...
        severities = [topology.compute_tracy_widom_severity(lam, mean=0.0, std=1.0) for lam in lambdas]

        # Check monotonicity
        for i in range(len(severities) - 1):
            assert severities[i] < severities[i + 1], f"Severity not monotonic: {severities[i]} >= {severities[i + 1]}"
...
class TestDriftContext:
    ...
    def test_build_creates_trajectory(self):
        ...
        # Add 10 samples
        for i in range(10):
            history.add_sample(drift_value=float(i) * 0.1, tracy_widom_severity=0.5, regime="STABLE")

        ctx = topology.build_drift_context(history=history, latest_lambda_max=1.0)
    ...
    def test_build_detects_turning_points(self):
        ...
        for i, regime in enumerate(regimes):
            history.add_sample(drift_value=float(i) * 0.2, tracy_widom_severity=0.5, regime=regime)

        ctx = topology.build_drift_context(history=history, latest_lambda_max=2.0)
    ...
    def test_build_empty_history(self):
        ...
        ctx = topology.build_drift_context(history=history, latest_lambda_max=0.0)
    ...
    def test_build_populates_all_fields(self):
        ...
        ctx = topology.build_drift_context(history=history, latest_lambda_max=1.5)
