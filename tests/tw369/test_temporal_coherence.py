...
def test_compute_drift_slope_decreasing():
    ...
    for i in range(5):
        buffer.add_event(f"Event {i}", drift_state={"drift_metric": 0.8 - i * 0.1})
...
def test_detect_regime_stability_stable():
    ...
    for i in range(5):
        buffer.add_event(f"Event {i}", tw_state={"regime": "stable", "plane": "6"})
...
def test_detect_regime_stability_unstable():
    ...
    for i, regime in enumerate(regimes):
        buffer.add_event(f"Event {i}", tw_state={"regime": regime, "plane": "6"})
