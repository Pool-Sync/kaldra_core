...
LEGACY_NAMES = {"Outlaw", "Jester", "Everyman", "Guardian", "Hermit", "Judge"}
    ...
    for path in FILES:
        ...
        for entry in data:
            ...
            for key in ("boost", "suppress"):
                ...
                for name in names:
                    # Must be canonical
                    assert name in CANONICAL, f"{path.name} entry {entry['id']}: invalid archetype '{name}' in {key}"

                    # Must NOT be legacy
                    assert (
                        name not in LEGACY_NAMES
...
def test_kindra_maps_no_legacy_names():
    ...
    for path in FILES:
        ...
        for legacy in LEGACY_NAMES:
            assert f'"{legacy}"' not in content, f"{path.name} contains legacy archetype name: {legacy}"
...
def test_kindra_maps_structure():
    ...
    for path in FILES:
        ...
        for entry in data:
            assert "id" in entry, f"{path.name}: entry missing 'id'"
            assert (
                "boost" in entry or "suppress" in entry
            ), f"{path.name} entry {entry['id']}: must have boost or suppress"

            # Check boost/suppress are lists
            if "boost" in entry:
                assert isinstance(entry["boost"], list), f"{path.name} entry {entry['id']}: boost must be a list"

            if "suppress" in entry:
                assert isinstance(entry["suppress"], list), f"{path.name} entry {entry['id']}: suppress must be a list"
