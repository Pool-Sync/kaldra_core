...
def test_layer_averages_not_zero_or_one():
    ...
    for layer in [ctx.layer1, ctx.layer2, ctx.layer3]:
        if len(layer.scores) > 0:
            ...
            assert 0.01 < layer.avg_score < 0.99, f"Layer avg_score {layer.avg_score} is saturated"
...
def test_tw_plane_distribution_reasonable_balance():
    ...
    assert all(v < 0.9 for v in dist.values()), f"TW-plane distribution too imbalanced: {dist}"
