    ...
    for key in ("boost", "suppress"):
        ...
        for name in names:
            if name not in ARCHETYPE_NORMALIZATION:
                raise ValueError(
                    f"Archetype '{name}' not in ARCHETYPE_NORMALIZATION table. Entry ID: {entry.get('id', 'unknown')}"
                )
...
def main():
    ...
    for path in FILES:
                ...
                for name in entry.get(key, []):
                    if name not in CANONICAL_SET:
                        raise AssertionError(
                            f"{path.name}: archetype '{name}' not canonical. Entry ID: {entry.get('id', 'unknown')}"
                        )
        ...
        print("  ✓ All archetypes canonical")
        print()
